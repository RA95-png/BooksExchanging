...
router.put('/EditeInformation' , (req , res)=>{
let name = req.body.username;
let phone = req.body.Mobile;
let social_media = req.body.SocialMedia;
let university =req.body.university ;
...
jwt.verify(token , key , (tokenFiled , resultOfToken)=>{
    ...
    if(tokenFiled) {
        ...
        UpdateInformationUser(id , name , phone , social_media , university,college,deparment ,  (EditeFiled , EditeSuccssed)=>{
            console.log(EditeFiled)
            if(EditeFiled){
                res.send({status:404})
            }else{
        let tokenUser = jwt.sign({id:id,name:name,phone:phone,social_media:social_media , university,college,deparment},key)
        res.send({
